...
* **Alur API oleh Frontend (Harus Sekuensial / Berurutan):**
  1. Tembak **`POST /admin/house`** untuk mendaftarkan rumah baru mereka ➔ Dapatkan `house_id` (dari `insertId` response).
  2. Tembak **`POST /admin/resident`** untuk mendaftarkan Kartu Keluarga baru ➔ Kirim `home: <house_id>` dari step 1. Dapatkan `family_id` (dari `insertId` response).
  3. Tembak **`POST /admin/datawarga`** untuk mendaftarkan Kepala Keluarga ➔ Kirim `fammilyId: <family_id>` dan `houseId: <house_id>`.
