 ...
 */
async function loginAccount(username, executor = pool) {
    const sqlcommand = `SELECT ${AUTH_COLUMNS_SQL} FROM acount WHERE username = ?`
    try {
        const [result] = await executor.execute(sqlcommand, [username])
        return result;
    } catch (err) {
 ...
 */
export async function getAccountById(id, executor = pool) {
 ...
 */
export async function getAccountByIdWithAuth(id, executor = pool) {
 ...
 */
export async function getAccountByBlindIndex(emailBlindIdx, executor = pool) {
    const sqlcommand = `SELECT ${AUTH_COLUMNS_SQL} FROM acount WHERE email_blind_idx = ?`
    try {
        const [result] = await executor.execute(sqlcommand, [emailBlindIdx])
        return result;
    } catch (err) {
