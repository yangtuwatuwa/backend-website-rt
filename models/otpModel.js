 ...
 */
export async function invalidatePreviousOtps(userId, purpose = 'VERIFICATION', conn = db) {
  const query = `
    UPDATE otp_codes 
    SET is_used = 1 
    WHERE user_id = ? AND purpose = ? AND is_used = 0
  `;
  const [result] = await conn.execute(query, [userId, purpose]);
  return result;
}
 ...
 */
export async function saveOtpCode(userId, otpHash, durationMinutes = 5, purpose = 'VERIFICATION', conn = db) {
  const query = `
    INSERT INTO otp_codes (user_id, otp_hash, purpose, expires_at) 
    VALUES (?, ?, ?, DATE_ADD(NOW(), INTERVAL ? MINUTE))
  `;
  const [result] = await conn.execute(query, [userId, otpHash, purpose, durationMinutes]);
 ...
 */
export async function getActiveOtp(userId, purpose = 'VERIFICATION', conn = db) {
  const query = `
    ...
    LIMIT 1
  `;
  const [rows] = await conn.execute(query, [userId, purpose]);
  return rows[0] || null;
}
 ...
 */
export async function incrementOtpAttempts(otpId, conn = db) {
 ...
 */
export async function markOtpAsUsed(otpId, conn = db) {
